...
$query = mysqli_query($conn, "SELECT t.*, u.nama FROM topup t JOIN users u ON t.user_id = u.id WHERE t.status != 'pending' ORDER BY t.tanggal DESC");
?>
<!DOCTYPE html>
<html lang="id">
<head>
    <meta charset="UTF-8">
    <title>Riwayat Top Up - Admin</title>
...
<div class="sidebar">
    ...
    <div class="nav flex-column">
        ...
        <a class="nav-link active" href="riwayat_topup_admin.php"><i class="bi bi-cash-stack me-2"></i> Riwayat Top Up</a>
        <hr>
        <a class="nav-link text-danger" href="logout.php"><i class="bi bi-box-arrow-left me-2"></i> Keluar</a>
    </div>
</div>
<div class="main-content">
    <h2 class="fw-bold mb-4">Histori Top Up User</h2>
    <div class="card border-0 shadow-sm p-3">
        <table class="table table-hover align-middle">
            <thead><tr><th>Tanggal</th><th>Nama User</th><th>Nominal</th><th>Metode</th><th>Status</th></tr></thead>
            <tbody>
                <?php while($row = mysqli_fetch_assoc($query)): ?>
                <tr>
                    <td><?= date('d/m/Y H:i', strtotime($row['tanggal'])); ?></td>
                    <td class="fw-bold"><?= $row['nama']; ?></td>
                    <td class="text-success fw-bold">Rp <?= number_format($row['nominal']); ?></td>
                    <td><?= $row['metode']; ?></td>
